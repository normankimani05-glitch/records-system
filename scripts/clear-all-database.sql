-- COMPLETE DATABASE CLEAR - FRESH START
-- This script removes ALL data from database and resets everything

-- Step 1: Disable foreign key constraints temporarily
SET session_replication_role = replica;

-- Step 2: Clear ALL tables in correct order (child tables first)
...
-- Clear credentials
TRUNCATE TABLE system_credentials RESTART IDENTITY CASCADE;
TRUNCATE TABLE staff_credentials RESTART IDENTITY CASCADE;
TRUNCATE TABLE vet_credentials RESTART IDENTITY CASCADE;

-- Clear other data
TRUNCATE TABLE notes RESTART IDENTITY CASCADE;

-- Step 3: Reset all sequences to start from 1
ALTER SEQUENCE IF EXISTS system_credentials_id_seq RESTART WITH 1;
ALTER SEQUENCE IF EXISTS staff_credentials_id_seq RESTART WITH 1;
ALTER SEQUENCE IF EXISTS vet_credentials_id_seq RESTART WITH 1;
...
-- Step 5: Reset auto-increment values for all tables (only for identity columns)
-- Only reset sequences for tables that actually have identity columns
DO $$
BEGIN
    -- Reset identity columns safely
    BEGIN
        ALTER TABLE system_credentials ALTER COLUMN id RESTART WITH 1;
    EXCEPTION WHEN OTHERS THEN
        -- Column is not identity, skip
    END;
    
    BEGIN
        ALTER TABLE staff_credentials ALTER COLUMN id RESTART WITH 1;
    EXCEPTION WHEN OTHERS THEN
        -- Column is not identity, skip
    END;
    
    BEGIN
        ALTER TABLE vet_credentials ALTER COLUMN id RESTART WITH 1;
    EXCEPTION WHEN OTHERS THEN
        -- Column is not identity, skip
    END;
...
SELECT '=== DATABASE COMPLETELY CLEARED ===' as status;
SELECT 'All tables truncated and reset successfully!' as message;
...
SELECT '=== FRESH START READY ===' as final_status;
SELECT 'Database is now completely empty and ready for fresh records!' as completion_message;
