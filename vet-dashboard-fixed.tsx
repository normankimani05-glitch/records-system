          ...
          <ImageModal isOpen={isModalOpen} image={modalImage} onClose={closeImageModal} />
              ...
              <CardContent>
                <div className="mb-6">
                  <Label htmlFor="treatment-records-select">Select Cow to View Treatment Records</Label>
                  <select 
                    id="treatment-records-select"
                    value={selectedCowTreatmentRecords || ""} 
                    onChange={(e) => setSelectedCowTreatmentRecords(e.target.value || null)}
                    className="block w-full p-2 border rounded-md"
                  >
                    <option value="">All Cows</option>
                    {COW_NAMES.map((cowName) => (
                      <option key={cowName} value={cowName}>{cowName}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-6">
                  {COW_NAMES.map((cowName) => {
                    if (selectedCowTreatmentRecords && selectedCowTreatmentRecords !== cowName) return null
                    const records = getRecordsForCow(cowName, 'treatment')
                ...
                {selectedCowTreatmentRecords && getRecordsForCow(selectedCowTreatmentRecords, 'treatment').length === 0 && (
                  <div className="text-center py-8 text-gray-500">
                    <Calendar className="w-12 h-12 mx-auto mb-2 opacity-50" />
                    <p>No treatment records for {selectedCowTreatmentRecords} yet</p>
                  </div>
                )}
              </CardContent>
            </Card>
